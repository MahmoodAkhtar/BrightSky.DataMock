...
public readonly record struct ParamValue
{
    ...
    internal ParamValue(string name, Func<object> mockTypeFactory) => (Name, MockTypeFactory) = (name, mockTypeFactory);
...
internal readonly record struct ParamValueTemplate
{
    ...
    internal List<string> ToList(int size = 100)
    {
        ...
        for (var i = 0; i < size; i++)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                var gta = t.GenericTypeArguments[0].FullName;
    ...
    private static string Format(string? gta, Type t, string formatted, string paramName, Dictionary<string, object> generatedValues, int i)
        => gta switch
        {
            "System.Bool" => WhenListOf(t, formatted, paramName, ((List<bool>)generatedValues[paramName])[i]),
            ...
            "System.Nullable`1[System.Bool]" => WhenListOf(t, formatted, paramName, ((List<bool?>)generatedValues[paramName])[i]),
